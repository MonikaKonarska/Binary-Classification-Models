...
Lets evaluate the model `model_lr_woe` performance. I used the train and also test and valid dataset to predict the target variable on unseen (test, valid) data. Predictions of the target variable are probabilities of default. Using this information allows us to evaluate the chosen threshold.

```{r glmPredictions, message = FALSE, warning = FALSE, echo = FALSE, include = FALSE}
source("3.feature_engineering.R")
source("functions.R")
load(file = file.path(dataPath, "dataToModeling.RData"))
dataTest  <- featurEngineering(data = dataTest)
dataTest  <- removeOutliers(data_set = dataTest)
dataValid <- featurEngineering(data = dataValid)
dataValid <- removeOutliers(data_set = dataValid)
...
train_predictions$predictions_lg_woe <- predict(model_lr_woe, newdata = dataSetWithVariablesCodedWoe, type = "response")
test_predictions$predictions_lg_woe <- predict(model_lr_woe, newdata = dataTestWithVariablesCodedWoe, type = "response")
valid_predictions$predictions_lg_woe <- predict(model_lr_woe, newdata = dataValidWithVariablesCodedWoe, type = "response")
...
Sometimes models automatically convert factor variables into binary dummy variables (for example glm() - logistic regression). while other models don`t. So after using logistic regression to predict default, I decided to use other algorithms to modeling. To do that it is worth creating some new variables - dummy variables.  
